...
MCP clients can drive. It lets a detection engineer (or an agent on
their behalf) take a Sigma rule, convert it to KQL targeting the Log
Analytics schema, statically validate the columns it touches, optionally
test-run it against a live Sentinel workspace, and emit an
